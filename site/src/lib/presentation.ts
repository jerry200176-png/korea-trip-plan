...
export function reservationLabel(deadline: string | null | undefined, required: boolean | null | undefined): string {
  if (required === true) return "需要預約";
  if (deadline) {
    const cleaned = sanitizeReaderText(deadline)
      .replace(/T-(\d+)\s*前?/g, "出發前 $1 天")
      .replace(/出發前 (\d+) 天\s*前/g, "出發前 $1 天");
    return `預約：${cleaned}`;
  }
  if (required === false) return "通常免預約（出發前再確認）";
  return "店家鎖定後補上";
}
